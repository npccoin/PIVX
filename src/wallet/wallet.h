...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that znpcspends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 NPC at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zNPC send
enum ZerocoinSpendStatus {
    ZNPC_SPEND_OKAY = 0,                            // No error
    ZNPC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZNPC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZNPC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZNPC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZNPC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZNPC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZNPC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZNPC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZNPC_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZNPC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZNPC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZNPC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZNPC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZNPC_SPENT_USED_ZNPC = 14,                      // Coin has already been spend
    ZNPC_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZNPC_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZNpcBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZNPCOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fznpcAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzNPCTracker> znpcTracker;
    ...
    void setZWallet(CzNPCWallet* zwallet)
    {
        zwalletMain = zwallet;
        znpcTracker = std::unique_ptr<CzNPCTracker>(new CzNPCTracker(strWalletFile));
    }

    CzNPCWallet* getZWallet() { return zwalletMain; }
    ...
    void setZNpcAutoBackups(bool fEnabled)
    {
    ...
    /** zNPC reset */
    boost::signals2::signal<void()> NotifyzNPCReset;
