...
public:
    ...
    bool WriteZNPCSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZNPCSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZNPCSeed_deprecated(uint256& seed);
    bool EraseZNPCSeed();
    bool EraseZNPCSeed_deprecated();

    bool WriteZNPCCount(const uint32_t& nCount);
    bool ReadZNPCCount(uint32_t& nCount);
